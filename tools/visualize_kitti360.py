...
def draw(
	...
	#? If not `idx_test`, set to 0
	# idx_test = 9
	# fov_voxels[fov_voxels[:, 3] != idx_test, 3] = 18
	# outfov_voxels[outfov_voxels[:, 3] != idx_test, 3] = 18
...
data_root = '/mnt/storage/SSCBenchKITTI360'
pred_root = '../../results/cgformer/CGFormer-Efficient-Swin-KITTI360-debug/sequences'
write_root = '../../results/cgformer/CGFormer-Efficient-Swin-KITTI360-debug/visualize'
...
intrin = np.identity(4)  # 4x4 matrix
intrin[:3, :4] = P2.reshape(3, 4)
lidar2cam = np.identity(4)  # 4x4 matrix
lidar2cam[:3, :4] = velo2cam[:3, :4]

voxels = os.listdir(os.path.join(pred_root, sequence, 'gt'))
voxels.sort()
...
print(len(voxels))
for voxel in voxels:
	save_root = voxel.split('.')[0]
	voxel_id = int(voxel.split('.')[0])
	# if voxel_id not in check_files:
	# 	continue
	...
	gt = np.fromfile(os.path.join(pred_root, sequence, 'gt', voxel), dtype=np.uint16)
	...
	occformer_img = draw(occ_gt, lidar2cam, vox_origin, fov_mask, 
				img_size=(1408, 384), f=707.0912, voxel_size=0.2, d=7, save_name=save_name_gt, save_root=save_root, video_view=False)
